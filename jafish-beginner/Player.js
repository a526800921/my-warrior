...
const __units = _units.map(item => {
  ...
  unit.defeatAmount = unit.atkDistance > 0 ? ceil(unit.HP / atk_3) : ceil(unit.HP / atk_5) // 击败或解救单位需要行动的次数

  // 主动攻击单位
  if (unit.atkDistance > unit.rely && unit.rely !== 0) {
...
const confirmes = [{ // 需要封装的api
  name: 'walk', // api名称
  options: { // 启用参数
    currentEncounterType: 0, // 更新遭遇状态
    defeatCount: false, // 当前单位进度+1
  },
}, {
  name: 'feel',
  options: {
  ...
  name: 'rescue',
  options: {
    currentEncounterType: 2,
    defeatCount: true,
  },
}, {
  name: 'attack',
  options: {
    currentEncounterType: 1,
    defeatCount: true,
  },
}, {
  name: 'look',
...
class Player {
  ...
  playTurn(warrior) {
    ...
    const unitList = spaceLook.map((item, index) => {
      if (item.isUnit()) {
        ...
        else if (unit.isWall()) return [3, index]
        return [4, index]
      } else {
        // 无单位
        return [0, index]
    ...
    if (firstUnit) {
      ...
      if (units[unitCount]) {
        ...
        const nextExtraHP = nextUnit && nextUnit.atkDistance >= nextUnit.rely && nextUnit.defeatAmount > 1 ? (((isForward ? nextUnit.health : nextUnit.backHealth) - 1) + (nextUnit.atk * nextUnit.atkDistance)) : 0
  ...
  addDefeatCount() {
    ...
    if (defeatCount >= units[unitCount].defeatAmount) {
      // 达到需要行动次数
  ...
  confirmed(warrior) {
    ...
    confirmes.forEach(item => warrior[`_${item.name}`] = () => {
      item.options.defeatCount && this.addDefeatCount() // 单位行动计数+1
      item.options.currentEncounterType && this.updateCurrentEncounterType(item.options.currentEncounterType) // 变更遭遇状态

      return isForward ? warrior[item.name].call(warrior) : warrior[item.name].call(warrior, 'backward')
    })
